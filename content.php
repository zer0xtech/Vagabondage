<?php
require('config.php');

if(empty($_SESSION['id'])) {
    header('Location: login.php');
}

?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="content.css">
    <title>Document</title>
</head>
<body>
    <div class="barrenav">
        <div class="SiteLogo">
            <a id="LienLogo" href="index.php"><img src="images\VagabondLogo.jpg" style="width: 100px; min-width: 33px;"></a>
        </div>
        <div>
            <a href="index.php">Accueil</a>
        </div>
        <div class ="BoutonRecherche">
            <input type="search" placeholder="Miyamoto, chapitre 328..."/>
            <button>Chercher</button>
        </div>
        <div>
            <a href="upload.php">Créer</a>
        </div>
        <div class="BoutonProfil">
            <a href="profile.php">Profil</a>
        </div>
    </div>

    <div class="gallery">
        <div class="publications">
            <div class="Images">
                <img src="images\OtsuClean.png" witdh="500px"; height="450px">
            </div>
            <div class="Username">
                <a href="profile.php">@KINGSULLY</a> 
            </div>
            <div class="Description">
                <p>#foryou #vagabond #otsu #trend #manga #miyamotomusashi #takezo #shinmentakezo #anitok #7kcc c#viral #vagabondmangaedit #matahachihoniden</p>
            </div>
            <div class="LikeNShare">
                <a href="#"><button>AIMER &#129655;</button></a>
                <a href="upload.php"><button>PARTAGER &#128227;</button></a>
            </div>
            <div class="Likes">
                <p><strong>Aimé par : </strong>@User, @User, @User, @User; @User, @User</p>
            </div>
            <div class="Commentaires">
                <input type="search" placeholder="Publier un commentaire :"/>
                <button id="BoutonCommentaire">POSTER</button>
            </div>
            <div class="EspaceCommentaires">
                    <p>@User : Type shit type shit<br>@User : Voilààà<br>@User : lorem.<br>@User : lorem<br>@User : des fois faut juste applaudir<br>@User : quel kiffe le projet</p>
            </div>
        </div>
</body>
</html>